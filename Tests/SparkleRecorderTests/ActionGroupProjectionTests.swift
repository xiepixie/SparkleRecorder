...
struct ActionGroupProjectionTests {
    ...
    func previewAffordanceSeparatesClickPulsesFromConditionRegions() {
        ...
        #expect(textClick == .locatorInputTarget)
        #expect(!textClick.showsClickPulse)
        #expect(!textClick.hasInteractiveAnchor)
        ...
        let pointSequence = ActionGroupProjection.previewAffordance(for: .multiPointClick)
        let drag = ActionGroupProjection.previewAffordance(for: .drag)
        let scroll = ActionGroupProjection.previewAffordance(for: .scroll)
        let locatorScroll = ActionGroupProjection.previewAffordance(for: .scroll, usesTextLocator: true)
        let move = ActionGroupProjection.previewAffordance(for: .mouseMove)

        #expect(pointSequence == .pointSequence)
        #expect(pointSequence.showsPath)
        #expect(!pointSequence.hasInteractiveAnchor)
        #expect(drag == .inputPath)
        #expect(drag.showsPath)
        #expect(drag.hasInteractiveAnchor)
        #expect(scroll == .scrollGesture)
        #expect(!scroll.showsClickPulse)
        #expect(!scroll.showsPath)
        #expect(!scroll.hasInteractiveAnchor)
        #expect(locatorScroll == .locatorInputTarget)
        #expect(locatorScroll.showsLocatorFallbackPoint)
        #expect(locatorScroll.showsTargetRegionLabel)
        #expect(move == .pointerMove)
        #expect(!move.showsClickPulse)
        #expect(!move.showsPath)
        #expect(!move.hasInteractiveAnchor)
    }
