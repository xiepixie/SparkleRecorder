...
struct AutomationQuickScheduleTests {
    ...
    func draftRoundTripPreservesTargetApplicationPolicy() throws {
        ...
        #expect(exported.document.workflow.tasks.first?.targetApplicationCleanupPolicy == "quitIfLaunched")
    ...
    func previewsRepeatingOccurrences() {
        ...
        #expect(preview.map(\.scheduledAt) == [
            startAt,
    ...
    func defaultsToDailySchedule() throws {
        ...
        let task = try #require(draft.makeWorkflow(
            for: macro,
            now: startAt.addingTimeInterval(-1)
        ).tasks.first)
        let expected = AutomationSchedule.repeating(AutomationRepeatRule(
            anchor: startAt,
            interval: .days(1)
        ))
    ...
    func weeklyScheduleUsesSelectedWeekday() throws {
        ...
        let startAt = try #require(calendar.date(from: DateComponents(
    ...
    func pastDailyTimeAnchorsToNextDay() throws {
        ...
        let selectedTime = try #require(calendar.date(from: DateComponents(
            year: 2026, month: 7, day: 15, hour: 9, minute: 0
        )))
        let savedAt = try #require(calendar.date(from: DateComponents(
        ...
        let task = try #require(AutomationQuickScheduleDraft(
            workflowName: macro.name,
            mode: .daily,
            startAt: selectedTime
        ).makeWorkflow(for: macro, now: savedAt).tasks.first)

        #expect(task.schedule == .repeating(AutomationRepeatRule(
            anchor: expected,
            interval: .days(1)
        )))
    }
    ...
    func summariesChooseEarliestNextRunAndCountDuplicates() throws {
        ...
        let summary = try #require(AutomationMacroScheduleSummary.summaries(
