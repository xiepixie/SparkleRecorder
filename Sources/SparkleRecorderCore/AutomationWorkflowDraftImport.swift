...
private struct DraftImportCompiler {
  ...
  func compile() -> AutomationWorkflowDraftImportResult {
    var issues =
      AutomationWorkflowDraftValidator
    ...
    let dependencies = expandedDocument.workflow.dependencies.compactMap {
      dependency -> AutomationDependency? in
      ...
      let dependencyID = stableID(
        for: "workflow:\(workflowID.uuidString):dependency:\(dependencyKey)")
  ...
  private func convertTask(
    ...
    return TaskConversion(
      task: AutomationTask(
        ...
        targetApplicationReadyDelay: task.targetApplicationReadyDelay ?? 0,
        targetApplicationCleanupPolicy: targetApplicationCleanupPolicy(
          task.targetApplicationCleanupPolicy
        ),
        targetApplicationQuitTimeout: task.targetApplicationQuitTimeout ?? 5,
        targetApplicationForceQuitOnTimeout: task.targetApplicationForceQuitOnTimeout ?? true,
  ...
  private func targetApplicationPolicy(_ value: String?) -> AutomationTargetApplicationPolicy {
    guard let rawValue = value?.trimmedForDraftImport,
      let policy = AutomationTargetApplicationPolicy(rawValue: rawValue)
    else {
  ...
  private func targetApplicationCleanupPolicy(
    ...
    guard let rawValue = value?.trimmedForDraftImport,
      let policy = AutomationTargetApplicationCleanupPolicy(rawValue: rawValue)
    else {
      return .keepOpen
    }
    return policy
  ...
  private func missedRunPolicy(_ value: String?) -> AutomationMissedRunPolicy {
    guard let rawValue = value?.trimmedForDraftImport,
      let policy = AutomationMissedRunPolicy(rawValue: rawValue)
    else {
      return .catchUp
    }
    return policy
  ...
  private func resolveMacro(
    ...
    if let name = macroRef.name?.trimmedForDraftImport, !name.isEmpty {
      ...
      let code: AutomationWorkflowDraftIssueCode =
        matches.isEmpty ? .missingMacroRef : .ambiguousMacroRef
      let message =
        matches.isEmpty
  ...
  private func conditionKind(
    ...
    case "externalSignal":
      let signalName =
        condition.text?.trimmedForDraftImport.nilIfEmptyForDraftImport ?? condition.regionRef?
        .trimmedForDraftImport.nilIfEmptyForDraftImport
  ...
  private func visualConditionKind(
    ...
    guard let type = AutomationVisualConditionType(rawValue: condition.type.trimmedForDraftImport)
    else {
      return nil
    }
  ...
  private func inferredResourceRequirement(
    ...
    case .macro:
      ...
      if let name = task.macroRef?.name?.trimmedForDraftImport,
        let entry = context.macroCatalog.first(where: {
          $0.name.caseInsensitiveCompare(name) == .orderedSame
        })
      {
        return resourceRequirement(for: entry.resourceRequirement)
...
private enum StableDraftImportUUID {
  static func make(namespace: String, key: String) -> UUID {
    ...
    let second = fnv1a64(
      text.reversed().map(\.asciiValueForDraftImport), seed: 0x8422_2325_cbf2_9ce4)
...
extension Character {
  fileprivate var asciiValueForDraftImport: UInt8 {
    unicodeScalars.first.map { UInt8(truncatingIfNeeded: $0.value) } ?? 0
  }
}

extension String {
  fileprivate var trimmedForDraftImport: String {
    trimmingCharacters(in: .whitespacesAndNewlines)
  }

  fileprivate var nilIfEmptyForDraftImport: String? {
    isEmpty ? nil : self
  }
}
