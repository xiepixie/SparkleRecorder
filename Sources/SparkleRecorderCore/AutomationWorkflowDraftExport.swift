...
private struct WorkflowDraftExportBuilder {
  ...
  func export() -> AutomationWorkflowDraftExportResult {
    ...
    let dependencies = workflow.dependencies.compactMap {
      dependency -> AutomationWorkflowDraftDependency? in
      guard let from = taskIDToKey[dependency.fromTaskID],
        let to = taskIDToKey[dependency.toTaskID]
      else {
        issues.append(
          issue(
            .warning,
    ...
    case .macro(let macroID):
      ...
      return AutomationWorkflowDraftTask(
        ...
        targetApplicationReadyDelay: task.targetApplicationReadyDelay == 0
          ? nil
          : task.targetApplicationReadyDelay,
        targetApplicationCleanupPolicy: task.targetApplicationCleanupPolicy == .keepOpen
          ? nil
          : task.targetApplicationCleanupPolicy.rawValue,
        targetApplicationQuitTimeout: task.targetApplicationCleanupPolicy == .keepOpen
          ? nil
          : task.targetApplicationQuitTimeout,
        targetApplicationForceQuitOnTimeout: task.targetApplicationCleanupPolicy == .keepOpen
          ? nil
          : task.targetApplicationForceQuitOnTimeout,
  ...
  private func draftRepeatInterval(_ interval: AutomationRepeatInterval) -> (
  ...
  private func visualRegion(
    ...
    regions.contains { region in
      region.key.trimmedForDraftExport == reference.trimmedForDraftExport && region.bounds == bounds
        && region.space == space
    }
  }
...
extension String {
  fileprivate var trimmedForDraftExport: String {
    trimmingCharacters(in: .whitespacesAndNewlines)
  }

  fileprivate var nilIfEmptyForDraftExport: String? {
    isEmpty ? nil : self
  }
}
