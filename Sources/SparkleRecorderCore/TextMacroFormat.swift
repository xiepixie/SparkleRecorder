...
///
/// Header (line 1):  `SPARKLERECORDER 1`
...
public enum TextMacroFormat {
    private static let headerPrefix = "SPARKLERECORDER"
    private static let legacyHeaderPrefix = String(decoding: [84, 73, 78, 89, 82, 69, 67, 79, 82, 68, 69, 82], as: UTF8.self)
    private static let currentHeader = "\(headerPrefix) 1"
    ...
    public static func export(_ events: [RecordedEvent]) -> String {
        ...
        for e in events {
            ...
            case .waitForText:
                lines.append("\(t)  WAITTEXT \"\(e.textAnchor?.text ?? "")\" TO \(e.textTimeout ?? 10.0)")
            case .verifyText:
                lines.append("\(t)  VERIFYTEXT \"\(e.textAnchor?.text ?? "")\" \(e.verifyMustExist ?? true ? "EXIST" : "NOTEXIST")")
            case .keyDown:
    ...
    public static func parse(_ text: String) throws -> MacroImportResult {
        ...
        guard let first = rawLines.first(where: { !$0.trimmingCharacters(in: .whitespaces).isEmpty }),
              isSupportedHeader(first) else {
            throw MacroImportError.notTextFormat("missing '\(headerPrefix)' header line.")
        }
        ...
        for (idx, raw) in rawLines.enumerated() {
            ...
            if !sawHeader {
                if isSupportedHeader(line) { sawHeader = true; continue }
            }
    ...
    private static func isSupportedHeader<S: StringProtocol>(_ line: S) -> Bool {
        let normalized = line.trimmingCharacters(in: .whitespaces).uppercased()
        return normalized.hasPrefix(headerPrefix) || normalized.hasPrefix(legacyHeaderPrefix)
    }
    ...
    private static func parseLine(verb: String, args: [String], time: TimeInterval) throws -> RecordedEvent? {
        ...
        case "WAITTEXT":
            guard args.count >= 3 else { throw ParseError.bad }
            let textStr = args[0].trimmingCharacters(in: CharacterSet(charactersIn: "\""))
            let timeout = Double(args[2]) ?? 10.0
            return RecordedEvent(kind: .waitForText, time: time, x: 0, y: 0, keyCode: 0, flags: 0, mouseButton: 0, clickCount: 0, scrollDeltaY: 0, scrollDeltaX: 0, textAnchor: TextAnchor(text: textStr, observedFrame: RectValue(x: 0, y: 0, width: 0, height: 0)), textTimeout: timeout)
        case "VERIFYTEXT":
            guard args.count >= 1 else { throw ParseError.bad }
            let textStr = args[0].trimmingCharacters(in: CharacterSet(charactersIn: "\""))
            var mustExist = true
            if args.count >= 2 && args[1].uppercased() == "NOTEXIST" { mustExist = false }
            return RecordedEvent(kind: .verifyText, time: time, x: 0, y: 0, keyCode: 0, flags: 0, mouseButton: 0, clickCount: 0, scrollDeltaY: 0, scrollDeltaX: 0, textAnchor: TextAnchor(text: textStr, observedFrame: RectValue(x: 0, y: 0, width: 0, height: 0)), verifyMustExist: mustExist)
        case "KEYDOWN":
