...
public enum AutomationEffect: Codable, Equatable, Sendable {
  ...
  case sendNotification(
    runID: UUID, workflowID: UUID, taskID: UUID, notification: AutomationNotificationSpec)
...
public enum AutomationReducer {
  ...
  public static func reduce(
    ...
    case .clockTick(let now):
      ...
      effects.append(
        contentsOf: expireResourceWaits(in: &state, now: now, environment: environment))
      ...
      effects.append(
        contentsOf: createDueScheduledRuns(in: &state, now: now, environment: environment))
      return effects
    ...
    case .moveTask(let workflowID, let taskID, let position, let at):
      state.now = at
      return moveTask(
        workflowID: workflowID, taskID: taskID, position: position, at: at, in: &state)
    ...
    case .deleteDependency(let workflowID, let dependencyID, let at):
      ...
      return deleteDependency(
        workflowID: workflowID, dependencyID: dependencyID, at: at, in: &state)
    ...
    case .runCreated(let run):
      ...
      return prepareRun(runID: run.id, in: &state, now: now, forceCheckpoint: true)

    case .manualStartWorkflow(let workflowID, let requestedAt):
      state.now = requestedAt
      guard let workflow = state.workflow(id: workflowID) else { return [] }
      let enabledTaskIDs = Set(workflow.tasks.filter(\.isEnabled).map(\.id))
      let rootTasks = workflow.tasks.filter { task in
        task.isEnabled
          && !workflow.dependencies.contains { dependency in
            dependency.isEnabled
              && dependency.toTaskID == task.id
              && enabledTaskIDs.contains(dependency.fromTaskID)
          }
      }
      guard !rootTasks.isEmpty else { return [] }

      let executionID = environment.makeRunID()
      return rootTasks.flatMap { task in
        createRun(
          workflowID: workflowID,
          taskID: task.id,
    ...
    case .playerStarted(let runID, let at):
      ...
      return [.persistRun(state.runs[index])]

    case .evidencePersistenceUpdated(let runID, let persistence, let at):
      state.now = at
      guard let index = state.runs.firstIndex(where: { $0.id == runID }) else {
        return []
      }
      state.runs[index].evidenceID = persistence.evidenceID
      state.runs[index].evidencePersistence = persistence
      return [.persistRun(state.runs[index])]
    ...
    case .playerFinished(let runID, let outcome, let at),
      ...
      state.now = at
      return completeRun(
        runID: runID, outcome: outcome, at: at, in: &state, environment: environment)
    ...
    case .persistenceSucceeded(let operation, let runID, _):
      guard let issue = state.persistenceIssue,
    ...
    case .persistenceFailed(let issue):
      state.persistenceIssue = issue
      guard issue.operation == .runCheckpoint,
        let runID = issue.runID,
        let index = state.runs.firstIndex(where: { $0.id == runID }),
        !state.runs[index].isTerminal
      else {
        return []
      }
      state.runs[index] = state.runs[index].completed(
        with: .rejected(reason: "Run data could not be saved: \(issue.message)"),
        at: issue.failedAt
      )
      return []
    }
  ...
  private static func expireTimedOutRuns(
    ...
    let timedOutRuns = state.runs.compactMap {
      run -> (runID: UUID, deadline: Date, createdAt: Date)? in
      guard
        !run.isTerminal,
        run.status == .queued || run.status == .running,
  ...
  private static func expireResourceWaits(
    ...
    let expiredWaits = state.runs.compactMap {
      run -> (runID: UUID, deadline: Date, createdAt: Date)? in
      guard
        !run.isTerminal,
  ...
  private static func createDueScheduledRuns(
    ...
    var effects: [AutomationEffect] = []
    let representedScheduleIndex = AutomationRepresentedScheduleIndex(runs: state.runs)
    ...
    for workflow in state.workflows {
      for task in workflow.tasks where task.isEnabled {
        ...
        let representedScheduledStarts = representedScheduleIndex.startTimes(
  ...
  private static func prepareRun(
    ...
    let originalRun = state.runs[index]
    ...
    if !workflow.dependencies(to: task.id).isEmpty {
      ...
      guard resolution.isSatisfied else {
        ...
        return checkpointEffects(
          ...
          current: state.runs[index],
          force: forceCheckpoint
        )
      }
      state.runs[index].earliestStartTime = maxDate(
        state.runs[index].earliestStartTime, resolution.earliestStartTime)
      state.runs[index].upstreamRunIDs = resolution.upstreamRunIDs
    }
    ...
    if !task.resourceRequirement.resources.isEmpty
      && !state.leases.contains(where: { $0.runID == runID })
    {
      state.runs[index].status = .waitingForResource
      return checkpointEffects(
        original: originalRun,
        current: state.runs[index],
        force: forceCheckpoint,
        followedBy: [.requestResource(runID: runID, requirement: task.resourceRequirement)]
      )
    }
  ...
  private static func startTask(
    ...
    case .delay(let duration):
      ...
      return [
        .persistRun(state.runs[index]),
        .wait(runID: runID, workflowID: workflow.id, taskID: task.id, duration: max(0, duration)),
      ]
    ...
    case .notification(let notification):
      ...
      return [
        .persistRun(state.runs[index]),
        .sendNotification(
          runID: runID, workflowID: workflow.id, taskID: task.id, notification: notification),
      ]
    }
  }

  private static func checkpointEffects(
    original: AutomationTaskRun,
    current: AutomationTaskRun,
    force: Bool,
    followedBy effects: [AutomationEffect] = []
  ) -> [AutomationEffect] {
    guard force || original != current else {
      return effects
    }
    return [.persistRun(current)] + effects
  }
  ...
  private static func downstreamRunID(
    ...
    state.runs
      .filter { run in
        run.workflowID == sourceRun.workflowID && run.taskID == dependency.toTaskID
          && run.executionID == sourceRun.executionID && run.upstreamRunIDs.contains(sourceRun.id)
      }
      .max { left, right in
  ...
  private static func runSortDate(_ run: AutomationTaskRun) -> Date {
    run.completedAt ?? run.actualStartTime ?? run.earliestStartTime ?? run.scheduledStartTime
      ?? run.createdAt
  }
  ...
  private static func resolveDownstream(
    ...
    var effects: [AutomationEffect] = []
    let targetTaskIDs = Set(
      workflow.dependencies(from: completedRun.taskID)
    ...
    for targetTaskID in targetTaskIDs.sorted(by: { $0.uuidString < $1.uuidString }) {
      ...
      if let existingIndex = state.runs.firstIndex(where: {
        $0.workflowID == workflow.id && $0.taskID == targetTaskID
        ...
        guard
          state.runs[existingIndex].status == .planned
            || state.runs[existingIndex].status == .waitingForDependencies
        else {
          continue
        }
        if task.joinPolicy == .firstMatched, !state.runs[existingIndex].upstreamRunIDs.isEmpty {
          effects.append(
            contentsOf: prepareRun(
              runID: state.runs[existingIndex].id, in: &state, now: completedAt))
          continue
        }
        ...
        let earliestStartTime = maxDate(
          task.schedule?.initialScheduledStart, resolution.earliestStartTime)
        ...
        effects.append(
          contentsOf: prepareRun(runID: state.runs[existingIndex].id, in: &state, now: completedAt))
        continue
      }
  ...
  private static func allDependencyResolution(
    ...
    return DependencyResolution(
      isSatisfied: true, earliestStartTime: earliestStartTime, upstreamRunIDs: upstreamRunIDs)
  }
  ...
  private static func singleDependencyResolution(
    ...
    return DependencyResolution(
      isSatisfied: true, earliestStartTime: match.readyAt, upstreamRunIDs: [match.run.id])
  }
  ...
  private static func lockedFirstMatchedDependencyResolution(
    ...
    guard !matches.isEmpty else {
      return DependencyResolution(
        isSatisfied: false, earliestStartTime: nil, upstreamRunIDs: upstreamRunIDs)
    }
  ...
  private static func dependencyMatch(
    ...
    guard
      let matchingRun = state.runs
        .filter({
          $0.workflowID == workflow.id && $0.taskID == dependency.fromTaskID
            && $0.executionID == executionID && $0.outcome.map(dependency.fires(for:)) == true
            && $0.completedAt != nil
        })
        .max(by: { lhs, rhs in
          (lhs.completedAt ?? .distantPast) < (rhs.completedAt ?? .distantPast)
        }),
      let completedAt = matchingRun.completedAt
    else {
      return nil
    }
