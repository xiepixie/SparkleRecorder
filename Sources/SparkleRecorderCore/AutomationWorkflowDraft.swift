...
public struct AutomationWorkflowDraftTask: Codable, Equatable, Sendable {
  ...
  public init(
    ...
    targetApplicationReadyDelay: TimeInterval? = nil,
    targetApplicationCleanupPolicy: String? = nil,
    targetApplicationQuitTimeout: TimeInterval? = nil,
    targetApplicationForceQuitOnTimeout: Bool? = nil,
    ...
    self.targetApplicationQuitTimeout = targetApplicationQuitTimeout
...
public enum AutomationWorkflowDraftLoopExpander {
  public static func expandedDocument(
    ...
    for task in document.workflow.tasks {
      ...
      guard let entry = copies.first?.key,
        ...
        let exitTrigger = expandedLoopExitTrigger(for: loop)
      else {
        continue
      }
      loopPorts[loopKey] = (
    ...
    expandedDependencies.append(
      contentsOf: generatedLoopDependencies(
        for: document.workflow.tasks
      ))
    expandedDependencies.append(
      contentsOf: document.workflow.dependencies.map { dependency in
        ...
        let trigger =
          fromLoop != nil && dependency.trigger.trimmedForDraftLoopExpansion == "success"
          ? fromLoop?.exitTrigger ?? dependency.trigger
          : dependency.trigger
        let key = dependency.key?.trimmedForDraftLoopExpansion.nilIfEmptyForDraftLoopExpansion.map {
          key in
  ...
  private static func expandedFixedLoopTasks(
    ...
    let loopKey = task.key.trimmedForDraftLoopExpansion
    let loopName =
      task.name?.trimmedForDraftLoopExpansion.nilIfEmptyForDraftLoopExpansion ?? loopKey
  ...
  private static func expandedRepeatUntilTasks(
    ...
    if repeatUntilNeedsManualApproval(loop) {
      tasks.append(
        repeatUntilManualApprovalTask(loopKey: loopKey, loopName: loopName, enabled: enabled))
    }
    ...
    for task in tasks {
      ...
      for iteration in 1...loop.count {
        for pairIndex in bodyKeys.indices.dropLast() {
          let from = expandedKey(
            loopKey: loopKey, iteration: iteration, bodyKey: bodyKeys[pairIndex])
          let to = expandedKey(
            loopKey: loopKey, iteration: iteration, bodyKey: bodyKeys[pairIndex + 1])
  ...
  private static func generatedRepeatUntilDependencies(
    ...
    for iteration in 1...attempts {
      ...
      if iteration < attempts {
        let nextFirst = expandedKey(
          loopKey: loopKey, iteration: iteration + 1, bodyKey: firstBodyKey)
        dependencies.append(
          AutomationWorkflowDraftDependency(
        ...
        dependencies.append(
          contentsOf: repeatUntilExhaustedDependencies(loopKey: loopKey, loop: loop, from: until))
      }
    }
  ...
  private static func expandedName(
    ...
    let bodyName =
      bodyTask.name?.trimmedForDraftLoopExpansion.nilIfEmptyForDraftLoopExpansion
  ...
  private static func repeatUntilNeedsManualApproval(_ loop: AutomationWorkflowDraftLoop) -> Bool {
    loop.normalizedFailurePolicyForDraftExpansion
...
private struct Validator {
  ...
  private mutating func validateVisualAssetKeys(
    ...
    for (index, rawKey) in keys.enumerated() {
      ...
      if key.isEmpty {
        add(
          .error, .invalidVisualAsset, "\(noun.capitalized) key is required.",
          "\(path)[\(index)].key")
      } else if !seen.insert(key).inserted {
        add(
          .error, .duplicateVisualAssetKey, "Duplicate \(noun) key '\(key)'.",
          "\(path)[\(index)].key")
      }
    }
  }
  ...
  private mutating func validateTasks() -> Set<String> {
    ...
    for (index, task) in document.workflow.tasks.enumerated() {
      ...
      if key.isEmpty {
        ...
        add(
          .error, .duplicateTaskKey, "Task key '\(key)' is duplicated.", "\(path).key", taskKey: key
        )
      }
  ...
  private mutating func validateTask(_ task: AutomationWorkflowDraftTask, path: String, key: String)
  {
    ...
    guard Self.supportedTaskTypes.contains(type) else {
      add(
        .error, .unsupportedTaskType, "Task type '\(task.type)' is not supported.", "\(path).type",
        taskKey: key)
      return
    }
    ...
    if let timeout = task.timeoutSeconds, timeout < 0 {
      add(
        .error, .invalidDuration, "Task timeout cannot be negative.", "\(path).timeoutSeconds",
        taskKey: key)
    }
    if let polling = task.pollingSeconds, polling < 0 {
      add(
        .error, .invalidDuration, "Task polling interval cannot be negative.",
        "\(path).pollingSeconds", taskKey: key)
    }
    if let threshold = task.condition?.threshold, !(0...1).contains(threshold) {
      add(
        .error, .invalidThreshold, "Condition threshold must be between 0 and 1.",
        "\(path).condition.threshold", taskKey: key)
    }
    if let retry = task.retry, let maxAttempts = retry.maxAttempts, maxAttempts < 1 {
      add(
        .error, .invalidRetry, "Retry maxAttempts must be at least 1.", "\(path).retry.maxAttempts",
        taskKey: key)
    }
    ...
    if let delay = task.targetApplicationReadyDelay, !(0...60).contains(delay) {
      add(
        .error,
        .invalidDuration,
        "Target application readiness delay must be between 0 and 60 seconds.",
        "\(path).targetApplicationReadyDelay",
        taskKey: key
      )
    }
    if let timeout = task.targetApplicationQuitTimeout, !(1...60).contains(timeout) {
      add(
        .error,
        .invalidDuration,
        "Target application quit timeout must be between 1 and 60 seconds.",
        "\(path).targetApplicationQuitTimeout",
        taskKey: key
      )
    }
  ...
  private mutating func validateMacroTask(
    _ task: AutomationWorkflowDraftTask, path: String, key: String
  ) {
    guard let macroRef = task.macroRef, !macroRef.isEmpty else {
      add(
        .error, .missingMacroRef, "Macro task '\(key)' needs a macroRef.", "\(path).macroRef",
        taskKey: key)
      return
    }
    ...
    if let id = macroRef.id {
      guard context.macroCatalog.contains(where: { $0.id == id }) else {
        add(
          .error, .missingMacroRef, "Macro ID '\(id.uuidString)' was not found.",
          "\(path).macroRef.id", taskKey: key)
        return
      }
    }

    if let name = macroRef.name?.trimmedForDraftValidation, !name.isEmpty {
      let matches = context.macroCatalog.filter {
        $0.name.caseInsensitiveCompare(name) == .orderedSame
      }
      if matches.isEmpty, macroRef.id == nil {
        add(
          .error, .missingMacroRef, "Macro named '\(name)' was not found.", "\(path).macroRef.name",
          taskKey: key)
  ...
  private mutating func validateConditionTask(
    _ task: AutomationWorkflowDraftTask, path: String, key: String
  ) {
    guard let condition = task.condition else {
      add(
        .error, .missingCondition, "Condition task '\(key)' needs a condition.",
        "\(path).condition", taskKey: key)
      return
    }
  ...
  private mutating func validateConditionPayload(
    ...
    if type == "ocrText", condition.text?.trimmedForDraftValidation.isEmpty ?? true {
      add(
        .error, .missingConditionText, "OCR text condition needs text.", "\(conditionPath).text",
        taskKey: taskKey)
    }
  ...
  private mutating func validateDelayTask(
    _ task: AutomationWorkflowDraftTask, path: String, key: String
  ) {
    guard let duration = task.delaySeconds else {
      add(
        .error, .missingDelayDuration, "Delay task '\(key)' needs delaySeconds.",
        "\(path).delaySeconds", taskKey: key)
      return
    }
    guard duration >= 0 else {
      add(
        .error, .invalidDuration, "Delay duration cannot be negative.", "\(path).delaySeconds",
        taskKey: key)
      return
    }
  }

  private mutating func validateNotificationTask(
    _ task: AutomationWorkflowDraftTask, path: String, key: String
  ) {
    guard let title = task.notification?.title.trimmedForDraftValidation, !title.isEmpty else {
      add(
        .error, .missingNotificationTitle, "Notification task '\(key)' needs a title.",
        "\(path).notification.title", taskKey: key)
      return
    }
  }

  private mutating func validateLoopTask(
    _ task: AutomationWorkflowDraftTask, path: String, key: String
  ) {
    guard let loop = task.loop else {
      add(
        .error, .invalidLoop, "Loop task '\(key)' needs a loop body.", "\(path).loop", taskKey: key)
      return
    }
  ...
  private mutating func validateLoopBody(
    ...
    guard !loop.tasks.isEmpty else {
      add(
        .error, .invalidLoop, "Loop task '\(key)' needs at least one body task.",
        "\(path).loop.tasks", taskKey: key)
      return
    }
    ...
    for (index, bodyTask) in loop.tasks.enumerated() {
      ...
      if bodyKey.isEmpty {
        add(
          .error, .emptyTaskKey, "Loop body task key is required.", "\(bodyPath).key", taskKey: key)
      } else if !bodyKeys.insert(bodyKey).inserted {
        add(
          .error, .duplicateTaskKey, "Loop body task key '\(bodyKey)' is duplicated.",
          "\(bodyPath).key", taskKey: key)
      }

      if bodyTask.type.trimmedForDraftValidation == "loop" {
        add(
          .error, .invalidLoop, "Nested loop tasks are not supported in draft v1.",
          "\(bodyPath).type", taskKey: key)
        continue
      }
  ...
  private mutating func validateSchedule(
    _ schedule: AutomationWorkflowDraftSchedule, path: String, taskKey: String
  ) {
    ...
    case "once":
      if schedule.startAt == nil {
        add(
          .error, .invalidSchedule, "Once schedule needs startAt.", "\(path).startAt",
          taskKey: taskKey)
      }
    case "repeating":
      if schedule.startAt == nil {
        add(
          .error, .invalidSchedule, "Repeating schedule needs startAt.", "\(path).startAt",
          taskKey: taskKey)
      }
      if (schedule.every ?? 0) < 1 {
        add(
          .error, .invalidSchedule, "Repeating schedule needs every >= 1.", "\(path).every",
          taskKey: taskKey)
      }
      if !["minutes", "hours", "days", "weeks"].contains(schedule.unit ?? "") {
        add(
          .error, .invalidSchedule,
          "Repeating schedule unit must be minutes, hours, days, or weeks.", "\(path).unit",
          taskKey: taskKey)
      }
    default:
      add(
        .error, .invalidSchedule, "Schedule type '\(schedule.type)' is not supported.",
        "\(path).type", taskKey: taskKey)
    }
  }
  ...
  private mutating func validateDependencies(taskKeys: Set<String>) {
    ...
    for (index, dependency) in document.workflow.dependencies.enumerated() {
      ...
      let dependencyKey =
        dependency.key ?? "\(dependency.from)->\(dependency.to):\(dependency.trigger)"
      ...
      if !taskKeys.contains(from) {
        add(
          .error, .missingDependencyEndpoint,
          "Dependency source '\(dependency.from)' does not match a task key.", "\(path).from",
          dependencyKey: dependencyKey)
      }
      if !taskKeys.contains(to) {
        add(
          .error, .missingDependencyEndpoint,
          "Dependency target '\(dependency.to)' does not match a task key.", "\(path).to",
          dependencyKey: dependencyKey)
      }
      if !from.isEmpty, from == to {
        add(
          .error, .selfDependency, "Dependency cannot point to the same task.", path,
          dependencyKey: dependencyKey)
      }
      ...
      if !Self.supportedTriggers.contains(trigger) {
        add(
          ...
          "Dependency trigger '\(dependency.trigger)' is not supported.", "\(path).trigger",
          dependencyKey: dependencyKey)
      }

      if let delay = dependency.delaySeconds, delay < 0 {
        add(
          .error, .invalidDuration, "Dependency delay cannot be negative.", "\(path).delaySeconds",
          dependencyKey: dependencyKey)
      }
      ...
      if !from.isEmpty, !to.isEmpty, !edges.insert(edgeKey).inserted {
        add(
          .warning, .duplicateDependency, "Dependency '\(edgeKey)' is duplicated.", path,
          dependencyKey: dependencyKey)
      }
  ...
  private mutating func validateCycles(adjacency: [String: [String]]) {
    ...
    for key in adjacency.keys.sorted() {
      if let cycleKey = visit(key) {
        add(
          .error, .cycleDetected, "Workflow dependencies contain a cycle at task '\(cycleKey)'.",
          "$.workflow.dependencies", taskKey: cycleKey)
        return
      }
    }
  }
...
extension String {
  fileprivate var trimmedForDraftValidation: String {
    trimmingCharacters(in: .whitespacesAndNewlines)
  }

  fileprivate var trimmedForDraftLoopExpansion: String {
    trimmingCharacters(in: .whitespacesAndNewlines)
  }
  ...
  fileprivate var nilIfEmptyForDraftLoopExpansion: String? {
    isEmpty ? nil : self
  }
}

extension AutomationWorkflowDraftLoop {
  fileprivate var isExpandableForDraftImport: Bool {
    if isFixedCount {
      return count > 0
    }
    if isRepeatUntil {
  ...
  fileprivate var normalizedFailurePolicyForDraftExpansion: String {
