...
struct MacroCard: View {
  ...
  var body: some View {
    styledCard
      ...
      .accessibilityAction(
        named: macro.favorite
          ? String(localized: "Remove favorite", table: "Common")
  ...
  func cardMenuItems(includePlayEdit: Bool) -> some View {
    ...
    if includePlayEdit {
      Button {
        ...
        Label(String(localized: "Play", table: "Common"), systemImage: "play.fill")
      }
      Button {
        onEdit()
      } label: {
        Label(
          String(localized: "Edit…", table: "Common"),
    ...
    Button {
      ...
      Label(
        String(localized: "Build Automation Sequence…", table: "Automation"),
    ...
    Button {
      ...
      Label(String(localized: "Rename…", table: "Common"), systemImage: "pencil")
    }
    Button {
      onToggleFavorite()
    } label: {
      Label(
        macro.favorite
          ? String(localized: "Unfavorite", table: "Common")
          : String(localized: "Favorite", table: "Common"),
        systemImage: macro.favorite ? "star.slash" : "star")
    }
    Divider()
    Button {
      onOpenNotes()
    } label: {
      Label(String(localized: "Notes…", table: "Common"), systemImage: "text.alignleft")
    }
    Button(action: onAddTag) {
      Label(String(localized: "Add Tag…", table: "Common"), systemImage: "tag")
    }
    Button {
      onAssignHotkey()
    } label: {
      Label(String(localized: "Assign Hotkey…", table: "Common"), systemImage: "keyboard")
    }
    if macro.hotkey != nil {
      Button {
        onClearHotkey()
        ...
        Label(
          String(localized: "Clear Hotkey", table: "Common"), systemImage: "keyboard.badge.ellipsis"
        )
      }
    }
    Divider()
    ...
    // Keep the contextual menu fast and task-focused. The ellipsis menu
    // remains the full customization surface for these library-wide lists.
    if !includePlayEdit {
      speedSubmenu()
      colorSubmenu()
      chainSubmenu()
    ...
    Button {
      ...
      Label(String(localized: "Duplicate", table: "Common"), systemImage: "plus.square.on.square")
    }
    Menu(String(localized: "Export", table: "Common")) {
      Button {
        ...
        Label(
          String(localized: "As SparkleRecorder File…", table: "Recording"),
          systemImage: "doc.badge.plus")
      }
      Button {
        onExportText()
      } label: {
        Label(String(localized: "As Text…", table: "EditorUX"), systemImage: "doc.plaintext")
      }
    }
    Divider()
  ...
  private var cardContent: some View {
    VStack(alignment: .leading, spacing: 7) {
      ...
      HStack(spacing: 6) {
        ...
        if isRenaming {
          TextField(
            String(localized: "Name", table: "Common"), text: $renameText, onCommit: onCommitRename
          )
        ...
        if let hk = macro.hotkey {
          ...
          .help(
            String(
              format: String(localized: "Hotkey: %@ — click to change", table: "EditorUX"), hk.name)
          )
          .accessibilityLabel(
            String(format: String(localized: "Change hotkey %@", table: "Common"), hk.name))
        }
        ...
        .help(
          macro.favorite
            ? String(localized: "Unstar", table: "Common")
            : String(localized: "Star", table: "Common")
        )
        .accessibilityLabel(
          macro.favorite
            ? String(localized: "Remove favorite", table: "Common")
        ...
        HStack(spacing: 4) {
          ...
          Button(action: {
            ...
            Label(String(localized: "Remove Target Window", table: "Recording"), systemImage: "xmark")
              ...
              .foregroundStyle(
                clearButtonHovered
                  ? AnyShapeStyle(Color.red.opacity(0.92)) : AnyShapeStyle(Color.secondary)
              )
              .frame(width: 19, height: 19)
              .contentShape(Circle())
          }
      ...
      if let automationSummary {
        Button(action: onSchedule) {
          HStack(spacing: 5) {
            Image(
              systemName: automationSummary.isEnabled
    ...
    HStack(spacing: 4) {
      ...
      Menu {
        ...
        Label(String(localized: "More actions", table: "EditorUX"), systemImage: "ellipsis")
          ...
          .foregroundStyle(
            moreButtonHovered ? AnyShapeStyle(Color.white) : AnyShapeStyle(Color.secondary)
          )
      ...
      .libraryControlSurface(
        cornerRadius: 8, tint: Brand.libraryBlue, isActive: moreButtonHovered,
        activeFillOpacity: 0.68
      )
  ...
  private var accessibilitySummary: String {
    ...
    if macro.playCount > 0 {
      parts.append(
        String(format: String(localized: "played %d times", table: "Common"), macro.playCount))
    }
    ...
    if let accentName {
      parts.append(
        String(
          format: String(localized: "color %@", table: "Common"), accentDisplayName(accentName)))
    }
    if macro.favorite {
    ...
    HStack(spacing: 4) {
      ...
      if macro.chainTo != nil {
        Image(systemName: "arrow.right")
          ...
          .help(
            chainTargetName.map {
              String(format: String(localized: "Chains to %@", table: "Common"), $0)
            } ?? String(localized: "Chained", table: "Common"))
      }
    }
    .lineLimit(1)
    .fixedSize()
