...
public enum MacroImportError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case .notRecFormat(let why): return "Not a recognized legacy Windows .rec file: \(why)"
...
// MARK: - legacy Windows .rec parser

public enum LegacyRecImporter {
    ...
    /// Parse a legacy Windows `.rec` file (flat array of 20-byte EVENTMSG records).
    public static func parse(_ data: Data) throws -> MacroImportResult {
        ...
        if recordCount > 0, Double(badHighWord) / Double(recordCount) > 0.5 {
            throw MacroImportError.notRecFormat(
                "the data does not look like legacy Windows EVENTMSG records.")
        }
        ...
        if recordCount > 0, Double(skipped) / Double(recordCount) > 0.05 {
            warning = "\(skipped) of \(recordCount) records were skipped — this file may be from an unsupported legacy recorder version."
        }
