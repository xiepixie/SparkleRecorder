...
enum AutomationProductEvidenceSnapshotScenario: String, CaseIterable {
  ...
  init?(argument: String) {
    ...
    case "running", "running-workflow":
      self = .running
    case "run-center", "runs", "run-history":
      self = .runCenter
    ...
    case "semantic-review-missing-artifacts", "review-missing-artifacts",
    ...
    case "semantic-review-materialized-actions", "review-materialized-actions",
      "materialized-review-actions":
  ...
  var selectedTaskID: UUID? {
    switch self {
    case .idle, .taskReorderAuthoring, .runCenter:
      return nil
    case .dragLinkAuthoring:
    ...
    case .editorPreviewAffordances, .templateBaselinePreviewRefs, .semanticReviewTimeline,
      .semanticReviewStoredBundle, .semanticReviewMissingArtifacts, .semanticReviewPixelColor,
      .semanticReviewMaterializedActions, .semanticReviewDraftPreview, .semanticReviewRunTarget:
      return nil
    }
  }
  ...
  var selectedRunID: UUID? {
    ...
    case .idle, .dragLinkAuthoring, .taskReorderAuthoring, .running, .runCenter,
      .editorPreviewAffordances, .templateBaselinePreviewRefs,
      .semanticReviewTimeline, .semanticReviewStoredBundle, .semanticReviewMissingArtifacts,
      .semanticReviewPixelColor, .semanticReviewMaterializedActions, .semanticReviewDraftPreview,
      .semanticReviewRunTarget:
      return nil
    }
  }
  ...
  var pendingDependencySourceID: UUID? {
    ...
    case .dragLinkAuthoring:
      return Self.fixedUUID("00000000-0000-0000-0000-00000000c202")
    case .idle, .taskReorderAuthoring, .running, .runCenter, .failedRunDetail,
      .failedRunPreviewUnavailable, .visualDiagnostics, .branchEvidence, .editorPreviewAffordances,
      .semanticReviewTimeline, .semanticReviewStoredBundle, .semanticReviewMissingArtifacts,
      .semanticReviewPixelColor, .semanticReviewMaterializedActions, .semanticReviewDraftPreview,
  ...
  var shouldAutoloadRunEvidence: Bool {
    ...
    case .failedRunDetail, .failedRunPreviewUnavailable:
      return true
    case .idle, .dragLinkAuthoring, .taskReorderAuthoring, .running, .runCenter, .visualDiagnostics,
      .branchEvidence, .editorPreviewAffordances,
      .templateBaselinePreviewRefs, .semanticReviewTimeline, .semanticReviewStoredBundle,
      .semanticReviewMissingArtifacts, .semanticReviewPixelColor,
      .semanticReviewMaterializedActions, .semanticReviewDraftPreview, .semanticReviewRunDetail,
      .semanticReviewRunTarget:
      return false
    }
  }
  ...
  var macroPackageFixtureDirectoryName: String {
    ...
    case .idle, .dragLinkAuthoring, .taskReorderAuthoring, .running, .runCenter, .failedRunDetail,
      .visualDiagnostics,
      .branchEvidence, .templateBaselinePreviewRefs, .semanticReviewTimeline,
      .semanticReviewStoredBundle, .semanticReviewMissingArtifacts, .semanticReviewPixelColor,
      .semanticReviewMaterializedActions, .semanticReviewDraftPreview,
  ...
  var taskListPreviewState: AutomationWorkflowTaskListPreviewState? {
    ...
    case .idle, .dragLinkAuthoring, .running, .runCenter, .failedRunDetail,
  ...
  func linkPreview(in workflow: AutomationWorkflowProjection?)
    -> AutomationFlowGraphLinkPreviewState?
  {
...
private struct SemanticRecordingPreviewRefsEvidenceView: View {
  ...
  private var header: some View {
    VStack(alignment: .leading, spacing: 10) {
      ...
      Text(
        "Fixture evidence for the accepted S0 -> S1 contract: source reference, runtime sample, and comparison decision render together from a SemanticRecordingBundle value model."
      )
    ...
    HStack(alignment: .top, spacing: 18) {
      if let template {
        railItem(
          ...
          detail:
            "Image template ref \(shortID(template.id)) from frame \(shortID(template.frameID))."
        )
      }
      railItem(
        title: "Bundle",
        ...
        detail:
          "Recording \(shortID(bundle.id)); \(bundle.frames.count) frames, \(bundle.visualObservations.count) observations."
      )
      railItem(
        title: "Safety",
        value: "safe relative artifact refs",
        detail:
          "SwiftUI renders ids and presenter-ready refs; app-edge code remains responsible for opening files."
      )
      railItem(
        title: "Trace",
...
private struct EditorPreviewAffordanceEvidenceView: View {
  ...
  private var evidenceRail: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      Spacer(minLength: 0)
      Text(
        "Fixture screenshot. It proves the editor overlay component and projection affordance mapping, not live recording capture."
      )
      .font(.system(size: 12, weight: .medium))
      .foregroundStyle(Color.white.opacity(0.54))
      .fixedSize(horizontal: false, vertical: true)
...
enum AutomationProductEvidenceSnapshotRenderer {
  static func render(
    ...
    if scenario == .runCenter {
      let view = AutomationRunCenterSheet(
        fixtureState: .ownerCFixture(now: now)
      )
    ...
    .environment(
      \.automationTaskRunEvidenceInitialActionFeedback, scenario.initialEvidenceActionFeedback
    )
    .background(Color(red: 0.10, green: 0.12, blue: 0.13))
  ...
  private static func semanticReviewMaterializedActionFixture(
    ...
    guard
      let candidate = projection.selectedFrame?.conditionCandidates.first(where: {
        $0.kind == .imageAppeared
      })
    else {
      throw SnapshotError.fixturePreparationFailed(
        "Semantic Review fixture did not expose an imageAppeared candidate.")
    }
  ...
  private static func seedSemanticReviewStoredBundleFrameArtifacts(
    ...
    for frame in bundle.frames {
      try writeEvidencePNG(
        ...
        size: CGSize(
          width: frame.imageSize?.width ?? 1_440, height: frame.imageSize?.height ?? 900),
  ...
  private static func semanticReviewArtifactStatuses(
    ...
    Dictionary(
      uniqueKeysWithValues: semanticReviewArtifactRefs(in: bundle).map { ref in
  ...
  private static func writeEvidencePNG(
    ...
    guard let tiffData = image.tiffRepresentation,
      ...
      throw SnapshotError.fixturePreparationFailed(
        "Could not encode semantic review stored-bundle PNG.")
    }
  ...
  private static func seedFailedRunEvidence(
    ...
    let macroID = AutomationProductEvidenceSnapshotScenario.fixedUUID(
      "00000000-0000-0000-0000-00000000c102")
    let evidenceID = AutomationProductEvidenceSnapshotScenario.fixedUUID(
      "00000000-0000-0000-0000-00000000c409")
    let evidenceURL =
      baseURL
    ...
    if writeUnreadableScreenshot {
      let data = Data(
        "not a decodable image; fixture intentionally exercises preview fallback".utf8)
  ...
  private static func drawLastSampleFixture(in rect: NSRect) {
    ...
    drawText(
      "last sample / display", at: NSPoint(x: 680, y: 88), size: 13,
  ...
  private static func drawRegionSampleFixture(in rect: NSRect) {
    ...
    drawText(
      "loading_spinner_template: not visible", at: NSPoint(x: 58, y: 94), size: 13,
      color: NSColor.systemOrange)
    drawText(
      "similarity 0.12 < 0.91", at: NSPoint(x: 58, y: 70), size: 13,
      color: NSColor.secondaryLabelColor)
  }
  ...
  private static func drawFailedRunScreenshotFixture(in rect: NSRect) {
    ...
    drawText(
      "Expected receipt confirmation", at: NSPoint(x: 132, y: 360), size: 16,
      color: NSColor.secondaryLabelColor)
    drawPanel(
    ...
    drawText(
      "No receipt was visible after event #3", at: NSPoint(x: 270, y: 236), size: 15,
      color: NSColor.systemPink)
    drawText(
      "failure screenshot / run C409", at: NSPoint(x: 652, y: 92), size: 13,
      color: NSColor.secondaryLabelColor)
  }
