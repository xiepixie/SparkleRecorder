...
struct AutomationQuickScheduleDraft: Equatable {
    ...
    func validationMessage(onOrAfter referenceDate: Date = Date()) -> String? {
        if normalizedWorkflowName.isEmpty {
            return String(localized: "The macro needs a name before it can be scheduled.", table: "Automation")
        }
...
struct AutomationMacroScheduleSummary: Equatable, Identifiable {
    ...
    static func summaries(
        ...
        let candidates = state.workflows.compactMap { workflow -> (UUID, AutomationWorkflow, AutomationTask, Date?)? in
            guard workflow.tasks.count == 1,
                  let task = workflow.tasks.first,
                  let macroID = task.kind.macroID,
                  let schedule = task.schedule,
                  schedule != .manual else {
                return nil
            }
            let nextRun = task.isEnabled
...
extension AutomationTargetApplicationPolicy {
    ...
    var detail: String {
        ...
        case .activateIfRunning:
            return String(localized: "Bring the bound application forward only when it is already open.", table: "Automation")
        case .launchIfNeeded:
            return String(localized: "Open the bound application and wait for its window before running.", table: "Automation")
        }
    }
}
...
extension AutomationSchedule {
    func previewOccurrences(startingAt referenceDate: Date, limit: Int = 3) -> [AutomationScheduledOccurrence] {
        guard limit > 0 else {
            return []
        }
