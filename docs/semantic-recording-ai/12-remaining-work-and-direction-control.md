...
| S2 capture | fake session、preflight、Settings preflight decision rows、retention、suppression、redaction planning、redacted frame PNG writing hook、Review/CLI redacted-frame preference、Recorder bridge first pass 已有 | 补真实 `.mov`、keyframe、bundle 产品证据；Settings/preflight live 产品证据仍 open |
...
| S4 CLI | fixture `recording list/show/explain/frames/frame show/events-near/ocr search/visual search/asset extract/asset baseline/suggest waits/conditions`、fixture/review-only `workflow draft from-recording`、explicit stored-bundle/default-root read-only `recording list/show/explain/frames/frame show/events-near/ocr search/visual search`、metadata-only stored-bundle `recording suggest waits/locators/conditions`、suggestion artifact-file/deleted status、explicit-source frame-region asset extraction、low-token transcript 和 no-evidence suggestion low-confidence guard 已有；S2 sidecar-aware loader/catalog entry point、default root 和 UUID directory -> manifest id consistency rule 已接入 S4 read-only CLI | 暂停 product-ready live work；live catalog/search/suggestions、cleanup suggestions、image-byte visual similarity、product-ready stored/live draft-from-recording 等授权 live bundle evidence、suggestion synthesis 和 Review/Draft Preview alignment |
...
| OCR/visual region picker | Partial fixture evidence; product gate open | `editor-preview-affordances.png` proves the overlay/projection mapping for wait/verify region labels versus click/text-click pulses; installed-app editor evidence and full grouping proof remain open |
...
| `recording ocr search` | Fixture, explicit stored-bundle and default-root read-only slices done; product-ready live open | 返回文字、bounds、confidence、query result ids、frame/source refs；S2 loader/catalog/root-id first pass 已接入，product-ready live CLI 等授权 bundle evidence、artifact status 和 wiring |
...
| `recording suggest ...` | Fixture waits/conditions and metadata-only stored-bundle waits/locators/conditions slices done; live/cleanup synthesis open; no-evidence summaries stay low confidence | fixture/stored 建议不突变宏，引用 evidence refs、confidence、risk、fallback、mutation policy；live/default-root、cleanup、missing-artifact 等 S2/S3 对齐后再接；缺少 evidence refs 的 suggestion summary 会被压到低置信度并追加 missing-evidence risk |
| `workflow draft from-recording` | Fixture/review-only first pass done; product-ready stored/live open | 产出 `sparkle.workflow.draft.v1`，仍走 validate/simulate/dry-run/import；product-ready stored/live synthesis 等 S2 live evidence、cleanup/missing-artifact status 和 Review/Draft Preview alignment |
...
1. 继续收口 OCR/visual region picker 的动作语义：fixture evidence 已证明等待/验证区域框和点击/text-click pulse 的 overlay 映射；下一步仍需 installed-app editor evidence、拖动/编辑不丢预览、以及“有意义等待/视觉变化不合并点击”的完整产品 proof。
...
7. S2 sidecar-aware bundle loading/catalog/root-id entry point 已完成 first pass 并已接入 S4 explicit stored-bundle/default-root CLI；S4 explicit-source asset extraction 已能写 package-local image/baseline refs，metadata-only stored-bundle suggestion 已能从 persisted observation 生成 Review-only proposals，suggestion payload 已能携带 artifact-file status，retention-pruned refs 已能通过 `.userDeleted` suppression 呈现为 `deleted`，fixture/review-only `workflow draft from-recording` 已能生成并验证/模拟 draft。下一步接 product-ready live catalog/search/suggestions 前仍需授权 live bundle evidence、installed-app saved macro links 和 cleanup suggestion policy；image-byte visual similarity 仍 open，product-ready stored/live `workflow draft from-recording` 需在 artifact status 和 Review/Draft Preview 边界稳定后再接。
...
- 2026-07-07 维护补齐 S4 low-token CLI transcript、default-root read-only CLI 和 suggestion-without-evidence safety gate：fixture CLI 能生成并验证/模拟/dry-run import 一个 review-only draft，默认 root 可以 read-only list/load；但 product-ready live catalog/search/suggestion synthesis、image-byte similarity 和 stored/live draft synthesis 继续保持 open。
