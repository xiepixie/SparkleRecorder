...
7. S2 sidecar-aware bundle loading/catalog/root-id entry point 已完成 first pass 并已接入 S4 explicit stored-bundle/default-root CLI；S4 explicit-source asset extraction 已能写 package-local image/baseline refs，fixture/review-only `workflow draft from-recording` 已能生成并验证/模拟 draft。下一步接 product-ready live catalog/search/suggestions 前仍需授权 live bundle evidence、installed-app saved macro links 和 suggestion synthesis；image-byte visual similarity 仍 open，product-ready stored/live `workflow draft from-recording` 需在 stored suggestion synthesis、artifact status 和 Review/Draft Preview 边界稳定后再接。
