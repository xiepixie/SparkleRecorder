...
Workflow orchestration now has a narrow draft-level loop first pass: `sparkle.workflow.draft.v1` can express a fixed-count loop body through `AutomationWorkflowDraftLoop` and expand it into an acyclic workflow through `AutomationWorkflowDraftLoopExpander` before simulation/import. The expansion maps condition/manual-approval body steps to `conditionMatched`, so wait-text loop bodies can advance to the next body step or next iteration. 2026-07-08 adds bounded Repeat-Until import: `kind = repeatUntil` requires `until` + `maxAttempts`, expands each attempt into body + until condition tasks, uses `conditionNotMatched` to continue and a `firstMatched` complete node to exit on any successful until condition. This does not close product loop acceptance. Repeating schedules and macro playback loops are still different concepts, dependency cycles are still rejected by validation, and future loop work still needs graph authoring, structured attempt evidence, foreach policy and live product proof instead of graph back-edges.
...
6. workflow-level loop semantics: fixed-count draft loop authoring/expansion and bounded Repeat-Until DAG expansion have first passes through Draft Preview/simulation/import; product authoring UI beyond current Review/Macro Editor entries, foreach, structured runtime attempt evidence and product proof remain open, and loops still cannot be represented by dependency cycles.
