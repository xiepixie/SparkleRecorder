...
| S4 CLI/AI | Fixture OCR/visual/search/suggestion, explicit stored-bundle read-only CLI, default-root read-only CLI/readiness/macro-link/artifact audit, stored-bundle metadata suggestion synthesis, suggestion artifact-file status, retention-pruned deleted-artifact status, explicit-source asset extraction, review-only draft-from-recording and low-token transcript first passes are done. | AI/CLI can query local evidence ids, audit bundle readiness/sidecar diagnostics/artifact file status including visual observation refs, distinguish retention/user-deleted artifact refs from ordinary missing refs through `.userDeleted` suppression evidence, audit saved macro -> semantic bundle references, extract selected frame assets, synthesize Review-only suggestions from persisted OCR/image/pattern/region/pixel observations, surface suggestion `artifactFiles`, and generate review-only drafts without sending video/image bytes or mutating workflows directly; default product-root commands no longer require temporary `--bundle-path` for read-only discovery. | Product-ready live catalog/readiness, cleanup suggestions, image-byte visual similarity, stored/live draft-from-recording and App Knowledge remain blocked on S2 live evidence, installed-app bundle links and Review acceptance semantics. |
...
| S4 | First pass paused for product-ready live work. Maintain fixture/explicit stored-bundle/default-root read-only CLI and persisted stored-bundle suggestion synthesis; resume live catalog/search/suggestion only after S2 live bundles and S3 Review boundaries stabilize. |
...
- Macro Editor behavior binding: bound behavior rows are treated as a single reusable event from the user's point of view. Duplicated behavior events now receive a fresh behavior id and a copied name instead of sharing the original id, the action row count chip now shows contained user-action count instead of raw event count, the sidebar has a dedicated Behavior section with separate `New Behavior` and `Selected Behavior` modes for naming/creating versus renaming/splitting/reusing, action-list context-menu Create/Unbind keeps selection on the newly merged or exposed rows, `Duplicate Behavior` now reselects the copied behavior as a new whole sequence, and Create now requires at least two event-backed recorded actions with no existing behavior in the selection.
- Draft loop authoring / preview explanation and visual provenance: fixed-count draft `loop` tasks can now be maintained through `AutomationWorkflowDraftEditor.setLoop` and CLI `workflow draft loop set --count <n> --tasks-json/--tasks-file`, then expanded to acyclic workflow steps before simulation/import and shown in Draft Preview as an explicit loop-expansion boundary. Bounded `repeatUntil` loop drafts can now preserve `until` condition, guardrails and failure policy through Codable/normalize/Preview, and import as ordinary acyclic body/until attempts when `maxAttempts` is present. S3 Review patch-builder proof can generate that bounded loop from a frame-crop visual candidate plus explicit body draft tasks, and the Review UI now has a linked-macro first slice that uses a unique `SavedMacro.semanticRecording` link as the loop `Do` body. Macro Editor now has a narrower OCR-text first slice: selected body actions plus one `Wait Text` / `Wait Text Gone` / `Verify Text` condition save a reusable behavior macro and open bounded Repeat-Until Preview. Draft Preview also has visual evidence rows that join Review-generated image/baseline/pixel conditions to source frame/surface/crop/search/threshold provenance before import. This proves the draft/import contract and narrow evidence-backed body sources; it does not introduce runtime graph cycles, image/pattern/region/pixel Macro Editor authoring, full body selection UI, product loop UI or structured runtime loop evidence yet.

This maintenance evidence supports the open action-preview and loop-semantics checklist items, but it does not close their full product gates. The remaining product proof still needs installed-app editor recording, click grouping proof across live meaningful waits/visual changes, broader Repeat-Until body picker beyond the narrow linked-macro/OCR-text paths, Macro Editor image/pattern/region/pixel until authoring, product loop UI/runtime evidence, repeat-until/foreach runtime policy, and installed-app semantic Review evidence after S2 produces live bundles.

The S2 storage boundary also advanced in this maintenance pass: `RecordingBundleStoreTests` now prove that a scratch-root app-edge store can write manifest/sidecar files, reload them through strict/tolerant APIs, catalog the canonical UUID directory, expose the stable default product root, reject explicit loads outside the configured root, and persist `.userDeleted` suppression evidence when retention pruning removes artifact files. `recording list --json` now reads that default root and returns the root path even when no bundle exists, `recording readiness` can audit fixture/explicit/default-root bundles with sidecar diagnostics, artifact file status and optional OCR/window/AX requirements, and `recording macro-links` can audit saved macro references, canonical relative paths and artifact files against the selected recording root. That closes the first-pass bundle-store/default-root/readiness/link-audit/deleted-artifact-status confidence item, but it does not prove live `.mov` or keyframe artifact production from ordinary recording.

No S2 live evidence has been accepted in this maintenance pass. S2 remains the only active unblocker for resuming S3 installed-app Review evidence and S4 product-ready live CLI/search/suggestion work. The new S4 stored-bundle suggestion path is deliberately metadata-only and Review-only; it does not replace accepted live bundle proof.
...
| Behavior binding UX | Duplicating a behavior creates a separate behavior group instead of reusing the original id; copied groups keep source context through `Copy of %@`. Behavior action-row count chips use contained user-action count (`2 actions`) instead of raw click/key down-up event count. The Macro Editor sidebar now has a dedicated Behavior section with `New Behavior` mode for naming/creating a whole-event block and `Selected Behavior` mode for renaming, splitting, or reusing the selected block through `Duplicate Behavior`. Rename disables and explains non-behavior selections, empty names and unchanged names through the same guard used by the submit/button path. Action-list context-menu Create reselects the newly merged behavior row, Duplicate Behavior reselects the copied behavior as one sequence, and Unbind reselects the exposed action rows so users keep editing context after regrouping. Create requires at least two event-backed recorded action rows, rejects selections that already contain behavior data, rejects non-contiguous recorded-action selections that skip over an unselected real action, and explains disabled states in the sidebar/context menu. | `Array<RecordedEvent>.duplicateEvents(at:)`, `Array<RecordedEvent>.renameBehavior`, `ActionGroup.containedActionCount`, `actionRowCountLabel`, `ActionGroupProjection.groups(containingEventIndices:)`, `ActionGroupProjection.behaviorGroups(containingEventIndices:excluding:groups:)`, `ActionListView.bindRows`, `ActionListView.unbindRows`, `EditorSidebar.behaviorSection`, `EditorSidebar.selectCopiedEvents`, `ActionGroupSelectionSnapshot.behaviorBindReadiness`, `behaviorBindReadinessHelp`, `BehaviorRenameReadiness`, `behaviorRenameReadinessHelp`, `MacroTransformerTimingTests.duplicatingBehaviorCreatesIndependentBehaviorGroup`, `MacroTransformerTimingTests.renamingBehaviorUpdatesEveryBoundEvent`, `ActionGroupProjectionTests.projectionFindsRegroupedBehaviorAndUnboundRows`, `ActionGroupProjectionTests.projectionFindsCopiedBehaviorGroupBeforeRawCopiedRows`, `ActionGroupProjectionTests.selectionSnapshotRequiresMultipleVisibleActionsToBindBehavior`, `EditorHelperTests.actionRowCountLabelKeepsBehaviorCountUserFacing`, `EditorHelperTests.behaviorBindReadinessHelpExplainsDisabledBinding`, `EditorHelperTests.behaviorRenameReadinessExplainsDisabledRenaming`, `EditorHelperTests.behaviorRenameReadinessHelpIsLocalized` and Macro Editor localization guard. | Fixes the post-recording behavior-block editing model. Installed-app editor recording of behavior copy/rename/create remains open. |
...
| Draft loop authoring / Draft Preview loop explanation | Fixed-count draft loops can be authored through the draft editor / CLI, then shown as an explicit expansion boundary before import while imported workflows remain acyclic. Bounded Repeat-Until drafts can now be preserved, explained, simulated and imported as acyclic body/until attempts when `maxAttempts` is present; S3 builder proof can generate them from a Review frame-crop candidate plus explicit body tasks; Review can now derive and choose a narrow `Do` body from linked `SavedMacro.semanticRecording` macros; Macro Editor can save selected body actions as a behavior macro and preview/import a bounded OCR-text Repeat-Until when exactly one text wait/verify condition is selected. | `AutomationWorkflowDraftEditor.setLoop`, CLI `workflow draft loop set`, `AutomationWorkflowDraftTests.draftEditorCanAuthorFixedLoopBody`, `AutomationWorkflowDraftTests.boundedRepeatUntilLoopExpandsToAcyclicSimulationAndImport`, `AutomationWorkflowDraftTests.repeatUntilLoopRequiresBoundedAttemptsBeforeImport`, `AutomationWorkflowDraftTests.draftSimulationHonorsFirstMatchedJoinPolicy`, `AutomationWorkflowDraftPreviewProjectionTests.previewProjectionExposesBoundedRepeatUntilExpansion`, `SemanticRecordingReviewProjectionTests.reviewImageCandidateCanCreateDraftOnlyRepeatUntilLoop`, `SemanticRecordingReviewProjectionTests.repeatUntilBodyResolverUsesOnlyEvidenceLinkedMacros`, `SemanticRecordingReviewProjectionTests.reviewRepeatUntilPatchCanUseResolvedMacroBody`, `MacroEditorRepeatUntilDraftBuilder`, `MacroEditorRepeatUntilDraftTests`, Draft Preview projection/UI and workflow draft loop tests. | Explains and maintains a draft/import contract. Broader Review/Macro Editor body picker beyond linked saved macros, Macro Editor image/pattern/region/pixel authoring, product loop UI, structured runtime attempt evidence, resource/evidence projection, foreach runtime policy and live loop proof remain future work. |
| Review stored-frame pixel sampling | Stored Review pixel candidates can now sample `colorHex` directly from the persisted frame artifact before creating `pixelMatched` Draft Patch / Repeat Until / accepted suggestion patches. Manual color picking still takes priority when the user has chosen a color. | `SemanticRecordingReviewPresenter.pixelColorSample`, `SemanticRecordingReviewFixtureView.pixelColorHexForDraft`, `SemanticRecordingReviewPixelSamplingTests` and `SemanticRecordingReviewProjectionTests.pixelConditionCandidateCanUseUserPickedColorWithoutMetadata`. | Closes a fixture/stored pixel-sampling proof path. Installed-app live pixel sampling, live frame-to-condition clip and accepted live bundle evidence remain open. |
| Review visual asset provenance rows | Review-generated image template and baseline assets preserve source frame id, surface id, source artifact path, crop bounds and crop coordinate space; visual conditions preserve search region and threshold; Draft Preview now renders joined image/baseline/pixel visual evidence rows before import. | `SemanticRecordingReviewDraftPatchBuilder`, `AutomationWorkflowDraftPreviewProjection.VisualAssetRow`, `AutomationWorkflowDraftPreviewSheet`, `AutomationWorkflowDraftPreviewProjectionTests.previewProjectionExposesVisualAssetProvenanceAndThresholds` and `SemanticRecordingReviewProjectionTests.regionBaselineCandidateWritesProvenanceAndPreviewsVisualEvidenceRow`. | Closes the stored Review/Draft Preview provenance checklist item. Installed-app live frame-to-condition product evidence remains open. |
...
| S2 bundle-store first pass / default-root/readiness/link-audit first pass | Scratch-root app-edge store writes manifest/sidecars, reloads strict/tolerant bundles, catalogs canonical UUID directories, exposes the stable App Support semantic recordings root, rejects root escape, read-only recording CLI can list/load/audit by id from the default root when no explicit source is provided, readiness reports artifact file status, retention pruning writes `.userDeleted` suppression sidecars for removed refs, and macro-link audit can check saved macro semantic recording references plus canonical relative paths. | `RecordingBundleStoreTests`, `SemanticRecordingArtifactFileAuditTests`, `SemanticRecordingCLITests`, `AutomationWorkflowDraftTests`, CLI smoke `SparkleRecorder recording list --json`, `SparkleRecorder recording readiness checkout-demo --fixture checkout --require-ocr --require-window-or-ax --json` and `SparkleRecorder recording macro-links --json`. | Closes store/default-root/readiness/link-audit/artifact/deleted-status first-pass confidence only. Real `.mov`, keyframes, OCR/window/AX and ordinary Recorder live evidence remain open. |
| S4 stored-bundle deterministic suggestions | Explicit stored-bundle suggestion queries can synthesize Review-only waits/locators/conditions from persisted OCR text, image/pattern observations, region baseline/diff observations and pixel samples. Suggestions carry deterministic ids, `availability: persistedBundle`, query filtering, confidence/risk/fallback and frame/event/observation/artifact evidence refs without reading image/video bytes or invoking Vision. Suggestion envelopes can now include `artifactFiles` status for bundle refs, including visual observation artifacts and retention/user-deleted refs. | `SemanticRecordingQueryEngine.persistedSuggestions`, `SemanticRecordingArtifactFileAuditor.summary`, `RecordingBundleStore.applyRetentionPlan`, `SemanticRecordingQueryTests.deterministicSuggestionsIncludePersistedBundleEvidenceProposals`, `SemanticRecordingCLITests.recordingSuggestionEnvelopeExposesPersistedStoredQueryContract`, `SemanticRecordingCLITests.recordingSuggestionEnvelopeSurfacesArtifactFileStatus`, `SemanticRecordingArtifactFileAuditTests.artifactAuditorReportsPresentEmptyAndMissingBundleFiles`, `SemanticRecordingArtifactFileAuditTests.artifactAuditorReportsUserDeletedRefsSeparatelyFromMissingFiles` and `RecordingBundleStoreTests.prunedArtifactsReloadAsUserDeletedArtifactEvidence`. | Closes the stored metadata suggestion and artifact/deleted-status first pass only. Cleanup suggestions, live/default-root product suggestions, image-byte similarity and product-ready stored/live draft-from-recording remain open. |
...
S4 already proves the low-token pattern: ids, safe refs, deterministic fixture queries, explicit stored-bundle reads, metadata-only stored-bundle suggestions and review-only draft output. The remaining gap is product scope, not command syntax.
...
- stored/live suggestion synthesis with clear `availability`, evidence refs, cleanup coverage and missing-artifact status
...
6. S4 resume product-ready live catalog/search/suggestion only after S2 live bundle readiness, installed-app saved macro links, cleanup suggestion policy and live missing/deleted artifact status are accepted.
...
- keep S4 fixture/explicit stored-bundle CLI, including persisted stored-bundle suggestions, as the low-token query path
...
5. Draft Preview handoff: package-local asset provenance、source frame/surface/crop/search/threshold 更容易读懂，避免用户以为 semantic bundle 内部文件就是长期 workflow 依赖。
