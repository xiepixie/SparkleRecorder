...
| Draft loop authoring / Draft Preview loop explanation | Fixed-count draft loops can be authored through the draft editor / CLI, then shown as an explicit expansion boundary before import while imported workflows remain acyclic. Draft-only Repeat-Until intent can now be preserved and explained without import/runtime activation; S3 builder proof can generate it from a Review frame-crop candidate plus explicit body tasks; Review can now derive and choose a narrow `Do` body from linked `SavedMacro.semanticRecording` macros; Macro Editor can save selected body actions as a behavior macro and preview a draft-only OCR-text Repeat-Until when exactly one text wait/verify condition is selected. | `AutomationWorkflowDraftEditor.setLoop`, CLI `workflow draft loop set`, `AutomationWorkflowDraftTests.draftEditorCanAuthorFixedLoopBody`, `AutomationWorkflowDraftTests.repeatUntilLoopStaysDraftOnlyAndDoesNotExpand`, `AutomationWorkflowDraftPreviewProjectionTests.previewProjectionExposesRepeatUntilAsDraftOnlyLoopIntent`, `SemanticRecordingReviewProjectionTests.reviewImageCandidateCanCreateDraftOnlyRepeatUntilLoop`, `SemanticRecordingReviewProjectionTests.repeatUntilBodyResolverUsesOnlyEvidenceLinkedMacros`, `SemanticRecordingReviewProjectionTests.reviewRepeatUntilPatchCanUseResolvedMacroBody`, `MacroEditorRepeatUntilDraftBuilder`, `MacroEditorRepeatUntilDraftTests`, Draft Preview projection/UI and workflow draft loop tests. | Explains and maintains a draft/import contract. Broader Review/Macro Editor body picker beyond linked saved macros, Macro Editor image/pattern/region/pixel authoring, product loop UI, runtime run evidence, resource/evidence projection, repeat-until/foreach runtime policy and live loop proof remain future work. |
