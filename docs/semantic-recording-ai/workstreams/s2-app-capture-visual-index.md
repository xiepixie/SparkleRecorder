...
- Settings now expands that switch into a SwiftUI preflight panel: it shows ready / blocked / degraded state from `SemanticRecordingPreflightPresentation`, renders issue rows, can re-check permissions, and opens the exact System Settings pane for the affected permission through `MenuBarController`
...
- bundle loading now has a first-pass stable sidecar and identity path: strict `RecordingBundleStore.loadBundle` reads `manifest.json`, overlays `video/segments.json`, `frames/index.jsonl`, `timeline.jsonl`, `events.jsonl`, `ocr/observations.jsonl` and `suppressed.jsonl` when present, falls back to manifest data when a sidecar is missing, and enforces UUID directory name -> manifest id consistency for catalog/default-root paths; explicit `loadBundleTolerant` returns `SemanticRecordingBundleLoadResult` with loaded/missing/failed sidecar diagnostics and falls back to manifest data when an optional sidecar is corrupt, giving S2 cleanup and later S4 live catalog/search a degraded-but-readable path. `SemanticRecordingBundleTests` cover pure merge, directory identity and tolerant diagnostics semantics, and `SemanticRecordingReviewPresenter` consumes the full strict loader instead of `loadManifest`
- bundle readiness now has a pure audit layer: `SemanticRecordingBundleReadiness` evaluates schema/reference validity, expected video/keyframe/timeline/AI-safe event presence, optional OCR/window/AX observation presence, frame-to-video alignment and redaction sidecar coverage for sensitive suppressions. `semantic-recording debug-smoke` now reloads the just-written bundle from disk through `RecordingBundleStore.loadBundleTolerant(from:)`, reports persisted reload counts plus memory-vs-persisted count match/mismatch details and loaded/missing/failed sidecar diagnostics in sidecar, plain summary and JSON `persistedBundleCountCheck`, and runs readiness against that persisted bundle rather than the in-memory finish value; blocked/preflight-only smoke runs leave reload/readiness unset. Reviewers can add `--require-ocr` and `--require-window-or-ax` when a live evidence gate must prove OCR or window/AX observations, rather than accepting a video/keyframe-only smoke.
...
- `SettingsPanel.swift` semantic preflight issue rows, re-check action and Open Settings buttons
...
- app-edge full bundle loading through strict `RecordingBundleStore.loadBundle` and explicit `loadBundleTolerant`, merging persisted sidecars into the manifest, enforcing UUID directory id consistency, reporting loaded/missing/failed sidecar diagnostics for degraded reads, and exposing `listBundleCatalog` for S4 explicit stored-bundle read-only CLI; product-ready default/live catalog still needs authorized live evidence and suggestion synthesis
...
- recording-start guidance: semantic preflight runs before popover dismissal/countdown, and blocked starts open the Settings preflight panel with issue rows and Open Settings actions
...
S4 fixture CLI is no longer blocked on S2, and S4 explicit stored-bundle read-only CLI now consumes S2 `RecordingBundleStore.loadBundle` and `listBundleCatalog`. S2 now has a first-pass UUID directory name -> manifest id consistency policy plus explicit tolerant load diagnostics for catalog/default-root paths. S4 product-ready default/live catalog/search/suggestions should still wait for authorized live bundle evidence, suggestion synthesis and a product decision about which default live roots are exposed.
...
- 2026-07-06: Added `RecordingBundleStore.applyRetentionPlan` as the app-edge retention application hook. It defaults to dry-run, reports planned/candidate/deleted/missing/preserved relative paths, deletes only safe non-directory artifact paths for prune plans, and can remove the canonical bundle directory for explicit deletion after confirmation. User-visible confirmation UI and storage-policy scheduling remain future S2 work.
