...
- 普通宏录制流程中启动 `SemanticRecordingCaptureSession`
...
1. Wire `SemanticRecordingCaptureSession` into the existing recording lifecycle behind a feature flag or internal entry point.
2. Surface preflight/degraded-mode result in the app shell before starting semantic recording.
3. Add real bundle smoke command or internal debug path that starts capture, records one event, finishes, writes bundle, and prints the bundle directory.
...
- 2026-07-06: Added `SemanticRecordingPreflight` value model/evaluator, live `PermissionCenter` bridge and `SemanticRecordingPreflightTests`; UI surfacing and semantic recording lifecycle gating remain open.
