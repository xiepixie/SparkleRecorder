...
1. Run `semantic-recording debug-smoke --json` on an authorized macOS 15+ machine and capture the resulting bundle directory as S2 live evidence.
2. Surface preflight/degraded-mode result in the app shell before starting semantic recording.
3. Wire `LiveSemanticRecordingSession` into the existing recording lifecycle behind a feature flag after debug-smoke evidence is captured.
