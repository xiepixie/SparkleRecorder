...
- live automatic detection that feeds suppression contexts from Secure Input, password fields and excluded targets during ordinary recording
- retention/deletion UI 和存储策略
- preflight result 的用户可见 SwiftUI 展示、Open Settings 映射和 degraded-mode guidance wiring
...
3. Wire `SemanticRecordingPreflightPresentation` into the app shell before starting semantic recording, including Open Settings mapping at the app edge.
...
6. Feed live Secure Input/password/excluded target detection into `SemanticRecordingSuppressionContext` during ordinary recording.
