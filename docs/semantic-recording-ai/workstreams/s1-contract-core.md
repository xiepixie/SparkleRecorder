...
S4 requests a complete authoring schema and executable capability boundary; S2/S3 request normalization and stable identity for storage/testing. Accepted: `MacroCandidateDocument`, coverage/disposition types, `MacroCandidateCapabilities.current`, `MacroCandidateValidator.decode/normalize`, and `MacroCandidateIdentity.revision`. Source metadata/configuration is protected. Candidate events are broadly replaceable and may reference any existing Source Revision Playback Surface. External v4 authoring omits `macro.surfaces` entirely and selects existing surfaces through `event.surfaceId`; minimized Surface context is supplied separately through `source-context.json` and `reconstruction.json`. App-owned Candidate Draft rebinding is explicitly authorized separately. Every candidate text operation has an explicit Playback Surface, and locator-driven pointer gestures keep one locator identity from down through up. Every source action has a reasoned disposition and real target IDs, scoped to literal candidate revision `candidate`. Unresolved/uncertain actions remain testable; promotion needs separate acknowledgement. Optional reconstruction provenance preserves old bundle decoding without invented clock anchors. Direct suites: `MacroCandidateValidatorTests`, `RecordingReconstructionProvenanceTests`, prior reconstruction clock/geometry/action/projection suites.
