...
Recommended order from here: S0 strict evidence is closed. S2 should continue authorized live semantic bundle work; S3 first pass is paused except for fixture/action-semantics maintenance until S2 provides live bundle + `SavedMacro.semanticRecording` inputs; S4 fixture OCR/metadata visual/suggestion CLI plus explicit stored-bundle/default-root read-only CLI are done, so S4 product-ready live catalog/search/suggestion work should wait for authorized live bundle evidence, installed-app links and suggestion synthesis.
