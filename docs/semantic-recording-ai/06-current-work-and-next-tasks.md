...
| --- | --- | --- |
| Capture `.mov` recording | App | live adapter writes a video segment through `SCRecordingOutput` with start/end metadata |
| Capture event-aligned keyframes | App | fake/live adapter can write frame refs around start/click/text/wait/stop |
