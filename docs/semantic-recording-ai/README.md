...
[17-ai-assisted-macro-reconstruction-design.md](17-ai-assisted-macro-reconstruction-design.md) 是已接受、Slice 1 纯 Core 底座已实现的完整宏重建设计。它扩展早期 suggestion-only 方向：用户明确启动优化后，AI 可接收获准的完整视频证据并重写独立候选宏；普通 CLI 查询仍默认只返回 metadata，Workflow draft 合同保持独立。
...
- [17-ai-assisted-macro-reconstruction-design.md](17-ai-assisted-macro-reconstruction-design.md)：已接受、Slice 1 纯 Core 底座已实现的 AI 宏重建设计；以清洗后的 `SavedMacro` 作为操作骨架、MP4 作为视觉状态事实，补齐帧级时钟对齐和操作还原轨道，允许 AI 完整重写候选宏，并通过本地校验、一键试跑、原版本恢复和用户接受完成晋升。
