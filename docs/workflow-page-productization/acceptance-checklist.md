...
- [ ] OCR/视觉识别结果能作为后续编排输入，而不只是条件是否命中。
  - First pass: [x] `AutomationDependency.dynamicDelay` can read a duration from upstream condition evidence (`observedSummary` / diagnostic fields), clamp it with a maximum wait, fall back to the fixed delay when parsing fails, and drive downstream `earliestStartTime` through reducer dependency resolution. Projection labels show observed/fallback delay. This is a scoped dynamic-delay contract, not a general variable/expression system.
...
- [ ] Inspector 能编辑 schedule、timeout、retry、join policy、condition、dependency trigger。
  ...
  - First pass: [x] Selected condition tasks now surface the concrete condition definition before runtime/history/dependency details, expose OCR text, image appeared/disappeared, region changed, pixel matched, manual approval, external signal, and previous outcome as one `Condition type` control, and keep OCR/visual/pixel detail editors in the task inspector. Dependency inspector now labels edge editing as `LINK TRIGGER` / `Run when` and links back to the source condition task when applicable. Product evidence remains open.
  - First pass: [x] Task Inspector now edits the core fields for all existing runtime task kinds: macro source, delay duration, notification title/body/severity, condition definition, schedule, timeout, retry, join policy, and resource policy. Dependency trigger pickers in task authoring, dependency inspector, and FlowGraph linking toolbar are source-aware, so condition matched/not matched only appears when the source task is a condition. Full workflow loop authoring and live product evidence remain open.
  - First pass: [x] Right-side Task Inspector is now tabbed by user intent: `Block` for identity/task-kind definition, `Flow` for branches/dependencies/join/position, `Run` for manual run/schedule/status/history, and `Advanced` for timeout/retry/resource policy/delete. Dependency Inspector now separates `Link` source/target context from `Timing` trigger/delay editing. This reduces long-scroll editing but still needs live product evidence before the broader UI gate closes.
  - First pass: [x] OCR/visual Draw Region authoring now captures the display before the overlay through the shared `AutomationScreenRegionPicker`, shows the selected real crop, detected top-window summary, and original pixel dimensions in Task Inspector, resolves `.automatic` to content/window-normalized bounds when available, and saves draw/clear region actions immediately. pixelMatched can sample normalized X/Y and target color by clicking the crop preview. AI Draft baseline capture uses the same screen-region picker and persists only its explicit baseline asset; ordinary preview screenshots remain transient authoring UI, not durable workflow payloads or live run artifacts.
  - First pass: [x] Task Inspector Advanced now edits resource policy fields through `.upsertTask`: foreground input, screen capture, accessibility, network, priority, and max resource wait. OCR/visual condition tasks lock required screen-capture resources while still exposing the policy so users can understand why a task waits. This is app-side authoring only; priority/preemption policy, structured loop runtime UI, and live installed-app evidence remain separate gates.
...
- [ ] OCR/视觉条件识别出的倒计时能决定下游任务下一次执行时间。
  - First pass: [x] Dependency links from OCR/visual condition tasks can enable `Use recognized time`, parse unit-labelled or colon-formatted durations from condition evidence, use a fallback delay when unavailable, cap the maximum wait, and project observed/fallback labels on graph edges and branch/dependency rows. Product-ready live evidence and user-selectable capture groups/formats remain open.
- [ ] 区域变化或图标出现条件能触发下游。
