...
状态：验收目录；idle/running/drag-link/task-reorder/failed-run/preview-unavailable/branch/visual diagnostics/editor preview affordances/template-baseline preview refs/semantic review timeline/draft preview/materialized actions fixture evidence 和 semantic recording CLI low-token transcript 已补齐，真实 task reorder、branch consistency、visual diagnostics Open/Reveal 和 macro evidence Open/Reveal live clips 已补齐；真实 Macro Review / frame-to-condition 录屏仍属 S3 后续验收。
Owner：Owner 2, Product UI And Workflow UX

Semantic recording S0 现在使用本目录作为 Workflow Evidence Closure 的产品证据落点。S0 当前工作台在 `docs/semantic-recording-ai/workstreams/s0-workflow-evidence.md`；strict S0 live-product gate 当前为 13/13，但后续 UI 变更仍需继续补同类 evidence。
...
| `editor-preview-affordances.png` | Macro Editor preview affordance fixture 态 | `TargetCrosshairView` 能把 wait/verify 显示为带标签的 condition region，把 ordinary click / text click 显示为 click pulse；当前 artifact 是 fixture evidence，不声称 live installed-app editor 录屏。 |
...
| `semantic-review-draft-preview.png` | 语义录制 Review -> Draft Preview fixture 态 | S3 Review 生成的 frame-to-condition patch 能进入 `AutomationWorkflowDraftPreviewSheet`，并显示 package-local visual asset provenance、validate/simulate/dry-run import projection 和确认导入前的证据 refs。当前 artifact 是 fixture evidence；真实 installed-app Draft Preview/import 录屏仍属后续验收。 |
...
| `semantic-recording-cli-low-token-transcript.md` | 语义录制 CLI query / draft gate transcript | S4 CLI 能用 `recording explain`、OCR search、metadata-only visual search、review-only suggestions 和 fixture `workflow draft from-recording` 逐步查询本地证据，并通过 validate/simulate/import dry-run，而不默认输出图片 bytes 或完整视频。当前 artifact 是 fixture CLI evidence；product-ready default/live catalog/search/suggestion synthesis 仍属后续验收。 |
...
- visual diagnostics drill-in artifact: present as `visual-diagnostics-drill-in.png` with `visual-diagnostics-drill-in.md`. Runtime now persists `AutomationTaskRun.conditionEvidence` for OCR/visual condition runs, including observed summary, sample count, watched region, score/threshold when available, diagnostic fields, and optional last-sample / watched-region image artifact refs. The fixture screenshot proves last-sample and watched-region image preview loading through `AutomationConditionEvidenceArtifactPresenter`, plus Open/Reveal artifact affordances and inline action feedback. The live gate is also present as `live-visual-diagnostics-open-reveal.mov` with `live-visual-diagnostics-open-reveal.md`, captured from App-host run `4C6EF768-CD54-4779-8FAE-4C6DFD6077C9` and its App Support artifacts.
- branch evidence drill-in artifact: present as `branch-evidence-drill-in.png` with `branch-evidence-drill-in.md`. Branch live consistency evidence is also present as `live-branch-evidence-consistency.mov` with `live-branch-evidence-consistency.md`; this clip is assembled from a live App-host handoff run payload plus a live App window capture and closes the strict branch gate, while a richer manual Run Detail drill-in clip can still be recaptured later when automated AX/input capture is available.
- editor preview affordance artifact: present as `editor-preview-affordances.png` with `editor-preview-affordances.md`. It renders the real `TargetCrosshairView` against deterministic fixture overlay actions and proves wait/verify condition regions carry labels while click/text-click keep click pulse affordances. It does not claim live installed-app editor or semantic recording capture evidence.
...
- semantic review draft preview artifact: present as `semantic-review-draft-preview.png` with `semantic-review-draft-preview.md`. It proves a Review-generated frame-to-condition patch can open the normal Draft Preview sheet with visual asset provenance plus validate/simulate/dry-run import projection. It does not claim live installed-app Draft Preview or confirmed import.
...
- semantic recording CLI low-token transcript: present as `semantic-recording-cli-low-token-transcript.md`. It records fixture CLI `recording explain`, OCR search, metadata-only visual search, suggestion, `workflow draft from-recording`, validate, simulate and import dry-run commands returning compact evidence ids/safe refs without image or video bytes. It does not claim product-ready default/live recording CLI, stored/live suggestion synthesis or image-byte visual similarity.
- macro evidence live gate: present as `live-macro-evidence-open-reveal.mov` with `live-macro-evidence-open-reveal.md`, captured from App-host failed macro run/evidence `6C7D7143-D4AB-4C4F-AAC4-BBEB7D3B6B29` with per-run `report.json`, `manifest.json`, and `failure.png`.
...
S0 live-product evidence is now present. Each `.mov` / `.mp4` must remain a non-empty real recording in a supported video container; empty placeholder files or non-video files renamed to `.mov` / `.mp4` are reported as undersized/invalid and do not satisfy strict audit. The paired sidecar must name exactly one accepted `Clip file:` candidate, must describe `Evidence source:` as live recording/capture, not fixture/mock/synthetic evidence, and must keep `Checklist item:` tied to the matching live gate title and id:

- `live-visual-diagnostics-open-reveal.mov` with sidecar `.md` (present and satisfies the visual diagnostics live gate)
- `live-macro-evidence-open-reveal.mov` with sidecar `.md` (present and satisfies the macro evidence live gate)
- `live-branch-evidence-consistency.mov` or `.mp4` with sidecar `.md` (present and satisfies the branch live gate)
