...
- semantic review timeline artifact: present as `semantic-review-timeline.png` with `semantic-review-timeline.md`. It renders `SemanticRecordingReviewProjection` from `SemanticRecordingFixture.checkoutBundle`, including before/after frame ids, selected-frame OCR overlay, source/runtime comparison, frame-to-condition Save Patch / Preview Draft affordances, suggestion evidence refs, S4-aligned `review.acceptSuggestion` / `review.rejectSuggestion` / `review.clearDecision` action contract, accepted-decision explanation and Accept Patch / Reject controls. It does not claim live installed-app frame-to-condition completion.
- semantic review stored bundle artifact: present as `semantic-review-stored-bundle.png` with `semantic-review-stored-bundle.md`. It writes a checkout semantic recording manifest plus PNG refs under `fixture-semantic-review-stored-bundles/<recording-id>/` and renders file-backed Available source/runtime/diff thumbnails in Review. It does not claim live installed-app capture or fake live video.
- semantic review missing artifacts artifact: present as `semantic-review-missing-artifacts.png` with `semantic-review-missing-artifacts.md`. It writes a checkout semantic recording manifest and frame PNGs while omitting source/runtime/diff/candidate files, proving Review keeps the safe evidence refs visible as `Missing file` and explains why Open/Reveal is unavailable. It does not claim live retention deletion or installed-app capture.
