...
状态：验收目录；idle/running/drag-link/task-reorder/failed-run/preview-unavailable/branch/visual diagnostics/template-baseline preview refs/semantic review timeline fixture PNG 已补齐，完整真实 drag/reorder、live capture 和真实 Macro Review 录屏仍缺。
...
| `semantic-review-timeline.png` | 语义录制 Review timeline fixture 态 | S3 fixture Review UI 能展示 event row -> before/after frame navigation、selected-frame OCR/source region overlay、frame-to-condition candidate、source/runtime decision 和 review-only suggestion policy。当前 artifact 是 fixture evidence；真实 Macro Review 接线和 frame-to-condition clip 仍属后续验收。 |
...
- semantic review timeline artifact: present as `semantic-review-timeline.png` with `semantic-review-timeline.md`. It renders `SemanticRecordingReviewProjection` from `SemanticRecordingFixture.checkoutBundle`, including before/after frame ids, selected-frame OCR overlay, source/runtime comparison, frame-to-condition candidate and review-only suggestion policy. It does not claim real Macro Review or live bundle presenter completion.
...
S0 live-product evidence still expected. Each `.mov` / `.mp4` must be a non-empty real recording; empty placeholder files are reported as undersized and do not satisfy strict audit. The paired sidecar must name exactly one accepted `Clip file:` candidate and must describe `Evidence source:` as live recording/capture, not fixture/mock/synthetic evidence:
...
`capture-plan` is the operator/agent checklist for closing S0: it lists missing live gates, accepted clip filenames, sidecar template commands, missing or invalid sidecar labels and undersized live clips. `prepare-live-capture` materializes the missing sidecar drafts before recording; it preserves existing notes by default, supports `--overwrite` for regeneration, and intentionally leaves placeholders that strict audit rejects until the real clip is captured and reviewed. `complete-sidecar` fills one reviewed sidecar through typed CLI fields, validates that `--clip` is an accepted filename for that gate, and still reports the gate open when the clip file is absent, empty, mismatched in `Clip file:`, or described as fixture/mock evidence. The normal audit reports fixture/live status and exits 0. The strict `--require-live` mode exits 1 until every S0 live-product artifact exists as a non-empty `.mov` / `.mp4` and the paired sidecar includes the required live capture labels: `Capture date:`, `Worktree note:`, `App build/run source:`, `Workflow/package:`, `User action:`, `Checklist item:`, `Known gaps:`, `Evidence source:`, and `Clip file:`. Current smoke result is 9/13 required items present: all fixture artifacts are present; the five live sidecar drafts exist; the four S0 live gates still miss clips and completed sidecar fields. The pure audit, capture-plan, sidecar-draft, sidecar-completion, empty-clip rejection, live-source validation and clip-match semantics are covered by `AutomationProductEvidenceAuditTests`.
