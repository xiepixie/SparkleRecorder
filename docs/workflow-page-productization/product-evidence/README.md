...
| `semantic-review-timeline.png` | 语义录制 Review timeline fixture 态 | S3 Review UI 能展示 event row -> before/after frame navigation、selected-frame OCR/source region overlay、frame-to-condition draft patch / Draft Preview affordance、source/runtime decision、suggestion evidence refs 和 Accept Patch / Reject review controls。当前 artifact 是 fixture evidence；Run Detail/live bundle presenter/Draft Preview handoff、pixel color picking 和 suggestion review first integration 已在代码中完成，但 live frame-to-condition clip、自动 run->bundle 绑定、installed-app product evidence 和 package materialization 仍属后续验收。 |
...
- semantic review timeline artifact: present as `semantic-review-timeline.png` with `semantic-review-timeline.md`. It renders `SemanticRecordingReviewProjection` from `SemanticRecordingFixture.checkoutBundle`, including before/after frame ids, selected-frame OCR overlay, source/runtime comparison, frame-to-condition Save Patch / Preview Draft affordances, suggestion evidence refs and Accept Patch / Reject controls. It does not claim live installed-app frame-to-condition completion.
