...
- Checklist item: S3 fixture Review projection, interactive Review UI, source/runtime evidence drill-in, suggestion review decisions and frame-to-condition patch affordance
...
- The inspector shows a frame-to-condition candidate with Save Patch and Preview Draft affordances, source/runtime decision, Source / Runtime / Diff evidence tiles, suggestion evidence refs, Accept / Reject controls, an accepted suggestion status, staged patch state and an undo review-decision control from the same `SemanticRecordingBundle` evidence ids.
