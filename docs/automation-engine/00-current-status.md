...
| Resource wait max timeout | First pass done | `AutomationResourceRequirement.maxWaitDuration`, reducer resource-wait expiry, draft `maxResourceWaitSeconds`, CLI `--max-resource-wait`, projection deadline/remaining/fraction, and Swift Testing coverage |
...
| Condition evaluation context | Done | `AutomationEffect.evaluateCondition` carries upstream `previousOutcomes`; `AutomationConditionEvaluatorClient.contextual` handles previous outcome, injected external signal/manual approval providers, OCR test closures, OCR region coordinate spaces, `LiveAutomationRuntimeHost` provider injection, app-layer frontmost window/content OCR context, durable `AutomationTaskRun.conditionEvidence`, optional live sample artifact refs, visual samples retained after capture even if bitmap decode fails, failure/rejected terminal diagnostics, and safe artifact presenter loading for UI preview/open/reveal |
