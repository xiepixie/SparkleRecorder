...
- 2026-09-07: Run Center asynchronous refreshes now use latest-generation publication for both runtime projections and storage usage. A slower silent poll cannot overwrite a newer runtime snapshot after loader return or after off-main projection work, and older storage success/failure results cannot replace a newer capacity state. Direct concurrency tests lock both latest-wins paths.
