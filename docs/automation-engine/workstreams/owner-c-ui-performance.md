...
- 2026-09-07: Owner C deepened Workflow Editor projection performance without changing reducer/runtime semantics. `AutomationRunHistoryIndex` centralizes latest Task, represented schedule-start, and same-Execution downstream lookups; Resource Timeline projects only the current execution context while Run Center keeps complete history; graph levels use O(T + D) DAG traversal with a bounded corrupt-cycle fallback. Changed overview/catalog/Run Center projection builds now run off `MainActor` with latest-generation publication and a separate working reducer state. Task Inspector defers Run History work until its Run tab and consumes a bounded five-row presentation while retaining total count, active Run, selected historical Run, and retry metadata. Direct coverage includes large Run History, overlapping projection builds, simultaneous active Executions, Timeline conflict semantics, and a 1,000-Task linear Workflow.
- 2026-09-07: Task Inspector authoring state is now grouped behind pure Core Draft Modules for schedule, execution, resources, and conditions instead of dozens of independent SwiftUI states and duplicate reset/materialization helpers. The App layer is split by authoring responsibility: `AutomationTaskConditionEditorView` owns OCR/visual picker and Playback Surface context presentation, while Flow, Run, and Advanced each have focused editor Modules. `AutomationTaskInspectorView` remains the shell that owns task identity, task-kind basics, and the single `.upsertTask` / delete lifecycle. Draft round-trip tests lock normalization, required-resource, and OCR/visual geometry semantics.
- 2026-09-07: Workflow import UI no longer constructs `AutomationVisualAssetPackageRootClient.fileBacked()` or duplicates package-root cleanup. Package import and AI Draft import both submit `AutomationVisualAssetPackageRootAssociation.Request` values to one App-edge Module, which centralizes root generation plus stale-root removal behind the injected Owner B client. SwiftUI retains only workflow/import state and error presentation.
- 2026-09-07: Workflow package import rollback now snapshots existing workflows at transaction start through a live provider instead of accepting the SwiftUI file-panel-open snapshot. `AutomationMainView` supplies `model.state.workflows`, so user edits made while the import panel is open become the rollback baseline and cannot be overwritten by an older workflow version if a later imported item fails. Package preflight likewise reads live Workflow IDs and live Macros after the file panel returns, then derives identity conflicts, duplicate imported IDs, missing Macro references, and Add Copies behavior from one `AutomationWorkflowPackageImportConflictPlan`. Draft Preview uses the same live Macro provider and resolves its validation/import catalog only after the draft file selection completes, so Macros added or removed while the panel is open cannot leave the preview on an older catalog. Direct coverage reproduces the stale-panel rollback race, import-time catalog changes, and Draft Preview catalog drift.
- 2026-09-07: Workflow package conflict resolution now evaluates current Workflow IDs after `NSOpenPanel` returns through the same live workflow provider used by transaction rollback. `AutomationWorkflowPackageImportConflictPlan` owns existing-ID and duplicate-package-ID detection plus Add Copies transformation, so Workflows created while the file panel is open cannot be silently replaced. Direct tests cover current conflicts, duplicate package IDs, copy semantics, package-directory preservation, and no-conflict imports.
- 2026-09-07: Workflow recording handoff timing is isolated in `AutomationWorkflowRecordingHandoff` instead of being implicit SwiftUI state. Each recording intent owns a generation UUID, the pre-recording Macro identity set, and its target Workflow; delayed completion checks must present that exact intent ID before they may select a newly recorded Macro or clear state. This prevents an older 250 ms completion task from consuming a newer recording intent while preserving current-Macro preference, newest-unseen fallback, and delayed Library publication behavior. Direct state-machine tests cover stale-token rejection and each completion path.
- 2026-09-07: Workflow authoring selection/link state is centralized in `AutomationWorkflowAuthoringState`. The Module owns Workflow/Task/Dependency/Timeline selection, Inspector Run selection, dependency-link begin/trigger/complete/cancel transitions, and repair of dangling Workflow/Task/Dependency/source references. A focused repair signature observes only Workflow IDs, Task IDs plus dependency-trigger vocabulary, and Dependency IDs, so topology changes repair immediately without re-running selection logic for unrelated name/schedule edits. `AutomationWorkflowListView` now receives selection read-only instead of holding a Binding that could bypass the Module. Direct tests cover dangling references, trigger normalization, existing/new dependency completion, and selection cleanup semantics.
- 2026-09-07: Run Center destructive confirmation is frozen at command creation through `AutomationRunCenterDeletionConfirmation`. The confirmation resolves the target Execution from the exact deletion `runIDs` and snapshots run count plus storage breakdown, so later filter/selection refreshes cannot make the alert describe a different Execution than the command will delete. Stale or non-deletion commands do not produce a confirmation. Direct tests cover exact Execution resolution and frozen storage context.
- 2026-09-07: Run Center refresh publication is latest-wins across both runtime snapshots and storage-usage reads. `AutomationRunCenterModel` assigns a generation before each accepted refresh and rechecks it after the loader and after off-main projection work, preventing slower silent polling from rolling back a newer runtime revision. Forced refreshes also supersede an in-flight loading request and own clearing inherited loading state, so a post-command refresh cannot be dropped behind an older pre-mutation snapshot. `AutomationRunCenterStorageUsageState` similarly rejects stale success/failure results, so startup/manual/post-deletion storage refreshes cannot overwrite newer capacity data. Direct gated tests cover these races.
- 2026-09-07: Run Center evidence navigation now treats `inspectEvidence(runID:)` command identity as authoritative. `AutomationRunCenterEvidenceSelection` resolves the exact Run across the full Run Center projection instead of consulting live selection, and only carries failure Task copy when the resolved Run is the current failure focus. This prevents refresh/selection drift from opening the wrong evidence context or labeling an unrelated Run with another Task's failure name.
- 2026-09-07: External Signal authoring now isolates provider reads and user writes behind `AutomationExternalSignalSourceState`. Each load captures trimmed signal identity plus a generation; changing signal or toggling locally invalidates older loads. `AutomationExternalSignalSourceView` uses a controlled Binding, so provider-driven UI refreshes no longer feed back through `.onChange` into `AutomationSignalStore`. Direct tests cover stale cross-signal loads, user edits winning over in-flight loads, empty names, and captured provider identity.
