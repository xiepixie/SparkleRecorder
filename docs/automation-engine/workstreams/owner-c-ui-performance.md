...
- 2026-09-07: Run Center refresh publication is latest-wins across both runtime snapshots and storage-usage reads. `AutomationRunCenterModel` assigns a generation before each accepted refresh and rechecks it after the loader and after off-main projection work, preventing slower silent polling from rolling back a newer runtime revision. `AutomationRunCenterStorageUsageState` similarly rejects stale success/failure results, so startup/manual/post-deletion storage refreshes cannot overwrite newer capacity data. Direct gated tests cover both races.
