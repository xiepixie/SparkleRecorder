...
- Accepted 2026-07-14: `AutomationPlayerStartRequest` also carries the task's target-application policy. The live adapter owns activation/launch and bounded window-readiness waiting before Player starts; `launchIfNeeded` failure returns a rejected outcome instead of allowing fallback clicks. App launch remains an app-edge concern and is fakeable in tests.
